...
class Heuristics:
    ...
    def addressTypeCluster(self, transactions_list):
        ...
        for trans in transactions_list:
            address = self.addressReuse(trans)
        return cluster
    ...
    def bip69Ferri(self, transactions_list):
        for trans in transactions_list:
            res = self.bip69(trans)
            if res == "no bip69":
                add = self.tresholdVote(trans, transactions_list)
                posicio = 0
                for i,outputs in enumerate(trans):
                    if i % 2 != 0:
                        for output in outputs:
                            if add == output[0]:
                                print(posicio)
